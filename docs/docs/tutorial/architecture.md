...
Conveyor CI provides intuitive primitives that we can use to accomplish this and these are, Resources, Drivers and Pipelines.

- **Resource**: The resource is a primitive that defines the state of the artifact you are working on. In our scenario it can contain the Git repository and other configurations like environment variables etc.
- **Driver**: These are programs whose logic carries out the actual execution of the CI/CD processes. For example setting up a container environment in which the flutter build will occur and then running the build process.
...
4. When the drivers receive the resource event, they then do their work respectively.
